...
static int mlk_print(char* msg){
 return printf("%s [%d -PPID=%d \n",msg,cp,P[cp].ppid);
}
...
static int mlk_send(char *mesg,int l, int p){
        P[p].msg=mesg;
         P[p].Lmsg=l;
        return 0;
}
static int mlk_recv(char *buff,int l)  {
        P[p].msg=buff;
         P[p].Lmsg=l;
     return  printf("%s /n",msg);
}
...
static void system_call() {
        ...
        switch (system_call_ctx->number) {
                ...
                case 3: r=mlk_sleep(system_call_ctx->u.i);
                        system_call_ctx->result=r;
                        break;
                case 4: r=mlk_print(system_call_ctx->u.s);
                	 system_call_ctx->result=r;
                	 break;
                case 5: r=mlk_signal(system_call_ctx->u.i);
                	 system_call_ctx->result=r;
                	 break;
                case 6: r=mlk_wait();
                	 system_call_ctx->result=r;
                	 break;
                case 7: r=mlk_send(system_call_ctx->u.s,system_call_ctx->u.log,system_call_ctx->u.i);
                	 system_call_ctx->result=r;
                	 break;
                case 8: r=mlk_recv(system_call_ctx->u.s,system_call_ctx->u.log);
                	 system_call_ctx->result=r;
                	 break;
...
static int choose_next_process() { // RR(1)
        ...
        int p=cp%nb_process;
        while (gettimeofday(&tv,NULL),P[p+1].alarm.tv_sec>tv.tv_sec || P[p+1].sleep==1) p=(p+1)%nb_process;
;
        P[p+1].quantum=1;
        return p+1;
}
